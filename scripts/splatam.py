...
def get_loss(params, curr_data, variables, iter_time_idx, loss_weights, use_sil_for_loss,
    ...
    im, radius, _, = Renderer(raster_settings=curr_data['cam'])(**rendervar) # opacity, _ = Renderer(raster_settings=curr_data['cam'])(**rendervar)
...
def add_new_gaussians(params, variables, curr_data, sil_thres, 
    ...
    if torch.sum(non_presence_mask) > 0:
        ...
        if monocular:
            ...
            rendervar = transformed_params2rendervar(params, transformed_gaussians) 
            _, _, _, = Renderer(raster_settings=curr_data['cam'])(**rendervar) #opacity, _ = Renderer(raster_settings=curr_data['cam'])(**rendervar) # TODO check if its optimal to render again
